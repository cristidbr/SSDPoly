...
@registry.BOX_HEADS.register('SSDCornerBoxHead')
class SSDCornerBoxHead(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.predictor = make_box_predictor(cfg)
    ...
    def _forward_test(self, cls_logits, bbox_pred):
        if self.priors is None:
            self.priors = PriorCornerBox(self.cfg)().to(bbox_pred.device)
        scores = F.softmax(cls_logits, dim=2)
        boxes = cornerbox_utils.convert_locations_to_corner_boxes(
            bbox_pred, self.priors, self.cfg.MODEL.COORDINATE_VARIANCE
        )
        #boxes = box_utils.center_form_to_corner_form(boxes)
        detections = (scores, boxes)
        detections = self.post_processor(detections)
